...
function buildEps({ reg, paintName, paintCode, bodyType, fkFont }) {
  ...
  const lines = [
    ...
    '',
    '% ================================================================',
    '% STEP 2 — Draw silhouette + letters as an EXPLICIT WHITE compound',
    '% path with even-odd fill. On clear vinyl the RIP treats 0 0 0 0',
    '% CMYK as "no ink" — so the pen barrel colour shows through cleanly.',
    '%',
    '% Even-odd rule preserves the visual hierarchy of nested shapes:',
    '%   Level 1 (body / letter outline) : WHITE (paint colour shows)',
    '%   Level 2 (wheel inside body /   ) :',
    '%           counter inside letter O) : NOT white — the K black',
    '%                                      from step 1 stays visible',
    '%   Level 3 (wheel hub inside wheel): WHITE again',
    '% ================================================================',
    '0 0 0 0 setcmykcolor',
    'newpath',
    '',
    '% Silhouette body + wheels + hubs + windows',
    silhouettePS,
    '',
    '% Registration',
    regPS,
